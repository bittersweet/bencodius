...
func TestDecodingInt(t *testing.T) {
	assert := assert.New(t)
	assert.Equal(Decode("i2e"), 2, "it should decode ints")
	assert.Equal(Decode("i-4e"), -4, "it should decode negatives")
}

func TestDecodingString(t *testing.T) {
	assert := assert.New(t)
	assert.Equal(Decode("4:girl"), "girl", "it should decode strings")
	assert.Equal(Decode("0:"), "", "it should decode empty strings")
}
...
func TestDecoding(t *testing.T) {
	...
	assert.Equal(Decode(string(data)), BencodeDict(OMap{
		...
		dict: map[BencodeString]BencodeValue{
			...
			"comment":       BencodeString("Downloaded from http://TheTorrent.org"),
			"created by":    BencodeString("uTorrent/3300"),
			"creation date": BencodeInt(1375363666),
			"encoding":      BencodeString("UTF-8"),
			"info": BencodeDict(OMap{
...
func TestEncoding(t *testing.T) {
	...
	assert.Equal(Encode(BencodeDict(OMap{
